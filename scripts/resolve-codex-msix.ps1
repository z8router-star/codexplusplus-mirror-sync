...
$cookieRelatesTo = [string]$cookieResponse.GetElementsByTagName('RelatesTo')[0].InnerText
$cookieExpirationNode = $cookieResponse.GetElementsByTagName('Expiration') | Select-Object -First 1
$cookieExpiration = if ($null -ne $cookieExpirationNode) { [string]$cookieExpirationNode.InnerText } else { $cookieExpires }
...
"@
$syncRequest = $syncRequest -replace '(?=<a:To s:mustUnderstand)', "<a:RelatesTo>urn:uuid:$cookieRelatesTo</a:RelatesTo>"
$syncRequest = $syncRequest -replace '<TicketType Name="MSA" Version="1.0" Policy="MBI_SSL">Retail</TicketType>', '<TicketType Name="MSA" Version="1.0" Policy="MBI_SSL"></TicketType>'
$syncRequest = $syncRequest -replace '<Expiration>2045-03-11T02:02:48Z</Expiration>', "<Expiration>$cookieExpiration</Expiration>"
