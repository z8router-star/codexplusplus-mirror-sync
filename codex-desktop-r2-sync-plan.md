...
[[checks]]
cmd = "python -c \"import pathlib,yaml; p=pathlib.Path('.github/workflows/sync.yml'); data=yaml.safe_load(p.read_text(encoding='utf-8')); jobs=data['jobs']; assert jobs['validate-r2-config']; assert jobs['download-windows']['needs']=='validate-r2-config'; assert 'if' not in jobs['mirror-codex-desktop-r2']; assert jobs['mirror-codexplusplus-r2']['needs']=='validate-r2-config'; assert str(jobs['mirror-codex-desktop'].get('if','')).strip().lower() in ('false','${{ false }}'); assert str(jobs['mirror-codexplusplus'].get('if','')).strip().lower() in ('false','${{ false }}'); smoke=yaml.safe_load(pathlib.Path('.github/workflows/gitee-api-smoke.yml').read_text(encoding='utf-8')); assert str(smoke['jobs']['verify-attachment-upload'].get('if','')).strip().lower() in ('false','${{ false }}')\""
