...
[[checks]]
cmd = "python -c \"import pathlib,yaml; p=pathlib.Path('.github/workflows/sync.yml'); data=yaml.safe_load(p.read_text(encoding='utf-8')); jobs=data['jobs']; main_guard='${{ github.ref == ' + chr(39) + 'refs/heads/main' + chr(39) + ' }}'; assert data['permissions']['contents']=='read'; assert 'publish-staging-release' not in jobs; assert jobs['validate-r2-config']; assert jobs['download-windows']['needs']=='validate-r2-config'; assert jobs['download-windows']['steps'][-1]['with']['compression-level']==0; assert all(jobs[name].get('if')==main_guard for name in ('validate-r2-config','download-windows','mirror-codex-desktop-r2','mirror-codexplusplus-r2')); assert jobs['mirror-codexplusplus-r2']['needs']=='validate-r2-config'; assert str(jobs['mirror-codex-desktop'].get('if','')).strip().lower() in ('false','${{ false }}'); assert str(jobs['mirror-codexplusplus'].get('if','')).strip().lower() in ('false','${{ false }}'); smoke=yaml.safe_load(pathlib.Path('.github/workflows/gitee-api-smoke.yml').read_text(encoding='utf-8')); assert str(smoke['jobs']['verify-attachment-upload'].get('if','')).strip().lower() in ('false','${{ false }}')\""

[[checks]]
cmd = "python -c \"import pathlib,yaml; jobs=yaml.safe_load(pathlib.Path('.github/workflows/sync.yml').read_text(encoding='utf-8'))['jobs']; expected={'R2_ACCOUNT_ID':'${{ secrets.CLOUDFLARE_ACCOUNT_ID }}','R2_ACCESS_KEY_ID':'${{ secrets.R2_ACCESS_KEY_ID }}','R2_SECRET_ACCESS_KEY':'${{ secrets.R2_SECRET_ACCESS_KEY }}','R2_BUCKET':'${{ vars.R2_BUCKET }}','R2_PUBLIC_BASE_URL':'${{ vars.R2_PUBLIC_BASE_URL }}'}; assert all(k not in jobs[name].get('env',{}) for name in ('mirror-codex-desktop-r2','mirror-codexplusplus-r2') for k in expected); desktop=next(step for step in jobs['mirror-codex-desktop-r2']['steps'] if step.get('name') == 'Publish raw R2 parts and stable pointer'); cpp=next(step for step in jobs['mirror-codexplusplus-r2']['steps'] if step.get('name') == 'Download, verify, and publish stable upstream assets'); assert all(desktop['env'].get(k)==v for k,v in expected.items()); assert all(cpp['env'].get(k)==v for k,v in expected.items())\""
